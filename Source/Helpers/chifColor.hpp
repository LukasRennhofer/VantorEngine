...
namespace chif::Helpers
{
    struct Color
	{
		...
		constexpr chif::Helpers::Math::Vec3 toFloat3() const
        {
            return chif::Helpers::Math::Vec3(
                ((rgba >> 0) & 0xFF) / 255.0f,
                ((rgba >> 8) & 0xFF) / 255.0f,
                ((rgba >> 16) & 0xFF) / 255.0f
            );
        ...
        constexpr chif::Helpers::Math::Vec4 toFloat4() const
        {
            return chif::Helpers::Math::Vec4(
                ((rgba >> 0) & 0xFF) / 255.0f,
                ((rgba >> 8) & 0xFF) / 255.0f,
                ((rgba >> 16) & 0xFF) / 255.0f,
                ((rgba >> 24) & 0xFF) / 255.0f
		...
		constexpr operator chif::Helpers::Math::Vec3() const { return toFloat3(); }
		constexpr operator chif::Helpers::Math::Vec4() const { return toFloat4(); }
		...
		static constexpr Color fromFloat4(const chif::Helpers::Math::Vec4& value)
		{
			return Color((uint8_t)(value.x * 255), (uint8_t)(value.y * 255), (uint8_t)(value.z * 255), (uint8_t)(value.w * 255));
		}

		static constexpr Color fromFloat3(const chif::Helpers::Math::Vec3& value)
		{
		...
		{
			return fromFloat4(chif::Helpers::Math::Lerp(a.toFloat4(), b.toFloat4(), i));
		}
