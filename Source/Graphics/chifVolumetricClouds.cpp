 ...
 * Date: 2025-03-08
 *
 * File: chifVolumetricClouds.cpp
 * Last Change: 
 */
 

#include "chifVolumetricClouds.h"
...
namespace chif::Graphics {
	VolumetricClouds::VolumetricClouds(int SW, int SH, chif::Graphics::CloudsModel * model): SCR_WIDTH(SW), SCR_HEIGHT(SH), model(model) {

		cloudsFBO = new chif::Graphics::Renderer::Buffer::TextureSet(SW, SH, 4);
		cloudsPostProcessingFBO = new chif::Graphics::Renderer::Buffer::FrameBufferObject(chif::Platform::Window::SCR_WIDTH, chif::Platform::Window::SCR_HEIGHT, 2);
	...
	void VolumetricClouds::draw() {
		...
		for (int i = 0; i < cloudsFBO->getNTextures(); ++i) {
			chif::Graphics::bindTexture2D(cloudsFBO->getColorAttachmentTex(i), i);
		}

		//Shader & cloudsShader = volumetricCloudsShader->getShader();
		chif::Graphics::Renderer::Shader::Shader & cloudsShader = *model->volumetricCloudsShader;
		...
		if (model->postProcess) {
			...
			chif::Graphics::Renderer::Shader::Shader &cloudsPPShader = model->postProcessingShader->getShader();
			...
			cloudsPPShader.setVec2("resolution", glm::vec2(chif::Platform::Window::SCR_WIDTH , chif::Platform::Window::SCR_HEIGHT));
