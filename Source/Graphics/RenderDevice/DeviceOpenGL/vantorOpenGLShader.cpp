...
namespace vantor::Graphics::RenderDevice::OpenGL
{
    ...
    Shader::Shader() {}
    // --------------------------------------------------------------------------------------------
    Shader::Shader(std::string name, std::string vsCode, std::string fsCode, std::vector<std::string> defines) { Load(name, vsCode, fsCode, defines); }
    // --------------------------------------------------------------------------------------------
    void Shader::Load(std::string name, std::string vsCode, std::string fsCode, std::vector<std::string> defines)
    {
        ...
        // if a list of define statements is specified, add these  to the start
        // of the shader source, s.t. we can selectively compile different
        // shaders based on the defines we set.
        if (defines.size() > 0)
        {
            std::vector<std::string> vsMergedCode;
            std::vector<std::string> fsMergedCode;
            // first determine if the user supplied a #version  directive at the
            // top of the shader code, in which case we  extract it and add it
            // 'before' the list of define code. the GLSL version specifier is
            // only valid as the first line of the GLSL code; otherwise the GLSL
            // version defaults to 1.1.
            std::string firstLine = vsCode.substr(0, vsCode.find("\n"));
            if (firstLine.find("#version") != std::string::npos)
            {
                // strip shader code of first line and add to list of shader
                // code strings.
            ...
            // then addremaining shader code to merged result and pass result to
            // glShaderSource.
            vsMergedCode.push_back(vsCode);
            fsMergedCode.push_back(fsCode);
            // note that we manually build an array of C style  strings as
            // glShaderSource doesn't expect it in any other format. all strings
            // are null-terminated so pass NULL as glShaderSource's final
            // argument.
        ...
        if (!status)
        {
            ...
            vantor::Backlog::Log("OpenGLShader", "Vertex shader compilation error at: " + name + "!\n" + std::string(log), vantor::Backlog::LogLevel::ERR);
        }
        glGetShaderiv(fs, GL_COMPILE_STATUS, &status);
        if (!status)
        {
            glGetShaderInfoLog(fs, 1024, NULL, log);
            vantor::Backlog::Log("OpenGLShader", "Fragment shader compilation error at: " + name + "!\n" + std::string(log), vantor::Backlog::LogLevel::ERR);
        }
        ...
        if (!status)
        {
            ...
            vantor::Backlog::Log("OpenGLShader", "Shader program linking error: \n" + std::string(log), vantor::Backlog::LogLevel::ERR);
        }
    ...
    int Shader::getUniformLocation(std::string name)
    {
        for (unsigned int i = 0; i < Uniforms.size(); ++i)
        {
            if (Uniforms[i].Name == name) return Uniforms[i].Location;
        }
        return -1;
    }
} // namespace vantor::Graphics::RenderDevice::OpenGL