...
* Date: 23.04.2025
*
* File: chifOpenGLRenderTarget.cpp
* Last Change:
*/

#include "chifOpenGLRenderTarget.hpp"
...
namespace chif::Graphics::RenderDevice::OpenGL {
    ...
    RenderTarget::RenderTarget(unsigned int width, unsigned int height, GLenum type, unsigned int nrColorAttachments, bool depthAndStencil)
    {
        ...
        {
            chif::Backlog::Log("OpenGLRenderTarget", "Framebuffer not complete!", chif::Backlog::LogLevel::ERR);
        }
    ...
    Texture* RenderTarget::GetColorTexture(unsigned int index)
    {
        ...
        {
            chif::Backlog::Log("OpenGLRenderTarget", "RenderTarget color texture requested, but not available: " + std::to_string(index), chif::Backlog::LogLevel::WARNING);
            return nullptr;
        }
    }
    // --------------------------------------------------------------------------------------------
