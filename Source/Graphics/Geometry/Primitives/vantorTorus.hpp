...
namespace vantor::Graphics::Geometry::Primitives {
    class Torus : public vantor::Graphics::RenderDevice::Mesh
    {
    public:
        Torus(float r1, float r2, unsigned int numSteps1, unsigned int numSteps2)
        {
            ...
            for (int i = 0; i <= numSteps1; ++i)
            {
                ...
                for (int j = 0; j <= numSteps2; ++j)
                {
                    ...
                    UV[i * (numSteps2 + 1) + j].x = ((float)i) / ((float)numSteps1) * glm::glm::two_pi<float>();
