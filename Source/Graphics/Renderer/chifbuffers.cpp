 ...
 * Date: 2025-03-08
 *
 * File: chifBuffers.cpp
 * Last Change: 
 */
 

#include "chifBuffers.h"
...
namespace chif::Graphics::Renderer::Buffer {
	...
	void unbindCurrentFrameBuffer() {
		glBindFramebuffer(GL_FRAMEBUFFER, 0);
		glViewport(0, 0, chif::Platform::Window::SCR_WIDTH, chif::Platform::Window::SCR_HEIGHT);
	}
	...
	TextureSet::TextureSet(int W, int H, int num)
	{
		if (W > 0 && H > 0 && num > 0) {
			...
			for (int i = 0; i < num; ++i) {
				texture[i] = chif::Graphics::generateTexture2D(W, H);
			}
		}
	}
	...
	{
		chif::Graphics::bindTexture2D(texture[i], unit);
	}

	void TextureSet::bind()
	{
		for (int i = 0; i < nTextures; ++i)
			chif::Graphics::bindTexture2D(texture[i], i);
	}
} // NAMESPACE chif::Graphics::Renderer::Buffer