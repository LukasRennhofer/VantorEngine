 ...
 *  Date: 2025-05-11
 *
 *  File: vantorWater.cpp
 *  Last Change: Automatically updated
 */

#include "vantorWater.h"
...
namespace vantor::Graphics
{
    Water::Water(glm::vec2 position, float scale, float height) : scale(scale), height(height)
    {
        ...
        // shad = new Shader("shaders/waterVertexShader.vert",
        // "shaders/waterFragmentShader.frag");
        shad = new vantor::Graphics::Renderer::Shader::Shader("WaterShader");
        shad->attachShader("shaders\\water.vert")->attachShader("shaders\\water.frag")->linkPrograms();
        ...
        reflectionFBO = new vantor::Graphics::Renderer::Buffer::FrameBufferObject(FBOw, FBOh);
        refractionFBO = new vantor::Graphics::Renderer::Buffer::FrameBufferObject(FBOw, FBOh);

        initializePlaneVAO(2, vantor::Graphics::Terrain::tileW, &planeVAO, &planeVBO, &planeEBO);
    }

    void Water::bindReflectionFBO() { reflectionFBO->bind(); }

    void Water::bindRefractionFBO() { refractionFBO->bind(); }
    ...
    void Water::draw()
    {
        ...
        // this->setPosition(glm::vec2(se->cam.Position[0],
        // se->cam.Position[2]), scale, height);
    ...
    {
        vantor::Graphics::Renderer::Buffer::unbindCurrentFrameBuffer(vantor::Platform::Window::SCR_WIDTH, vantor::Platform::Window::SCR_WIDTH);
    }
