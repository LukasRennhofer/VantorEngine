...
namespace chif::weather {
	VolumetricClouds::VolumetricClouds(int SW, int SH, chif::weather::CloudsModel * model): SCR_WIDTH(SW), SCR_HEIGHT(SH), model(model) {
		...
		cloudsFBO = new chif::Buffer::TextureSet(SW, SH, 4);
		cloudsPostProcessingFBO = new chif::Buffer::FrameBufferObject(Window::SCR_WIDTH, Window::SCR_HEIGHT, 2);
	...
	void VolumetricClouds::draw() {
		...
		for (int i = 0; i < cloudsFBO->getNTextures(); ++i) {
			chif::utils::bindTexture2D(cloudsFBO->getColorAttachmentTex(i), i);
		}

		//Shader & cloudsShader = volumetricCloudsShader->getShader();
		chif::Shader::Shader & cloudsShader = *model->volumetricCloudsShader;
		...
		if (model->postProcess) {
			...
			chif::Shader::Shader &cloudsPPShader = model->postProcessingShader->getShader();
