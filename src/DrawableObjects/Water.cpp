...
namespace chif::Render3D {
	Water::Water(glm::vec2 position, float scale, float height): scale(scale), height(height){
		...
		shad = new chif::Shader::Shader("WaterShader");
		...
		reflectionFBO = new chif::Buffer::FrameBufferObject(FBOw, FBOh);
		refractionFBO = new chif::Buffer::FrameBufferObject(FBOw, FBOh);

		initializePlaneVAO(2, chif::terrain::Terrain::tileW, &planeVAO, &planeVBO, &planeEBO);
	}
	...
	void Water::unbindFBO() {
		chif::Buffer::unbindCurrentFrameBuffer(Window::SCR_WIDTH, Window::SCR_WIDTH);
	}
