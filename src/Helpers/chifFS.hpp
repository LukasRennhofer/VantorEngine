...
namespace chif::Helpers::FileSystem {
    // Base functions
    ...
    #if defined(__SWITCH__)
        // ROMFS does not support directory iteration directly, so skip or implement your own index if needed.
        return files;
    #else
    ...
    inline bool FileExists(const std::string &path) {return chif::Platform::Environment::FileExists(path);}
} // chif::Helpers::FileSystem