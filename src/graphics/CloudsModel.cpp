...
namespace chif::weather {
	...
	CloudsModel::CloudsModel(sceneElements * scene, chif::Render3D::Skybox * sky) : scene(scene), sky(sky)
	{
		initVariables();
		initShaders();
	...
	{
		volumetricCloudsShader = new chif::Shader::Shader("volumetricCloudsShader", "shaders/volumetric_clouds.comp");
		postProcessingShader = new chif::Shader::ScreenSpaceShader("shaders/clouds_post.frag");

		weatherShader = new chif::Shader::Shader("weatherMap");
	...
	{
		if (!perlinTex) {
			chif::Shader::Shader comp("perlinWorley");
			comp.attachShader("shaders/perlinworley.comp");
			comp.linkPrograms();

			this->perlinTex = chif::utils::generateTexture3D(128, 128, 128);
		...
		if (!worley32) {
			chif::Shader::Shader worley_git("worleyComp");
			worley_git.attachShader("shaders/worley.comp");
			worley_git.linkPrograms();

			this->worley32 = chif::utils::generateTexture3D(32, 32, 32);
		...
		if (!weatherTex) {
			this->weatherTex = chif::utils::generateTexture2D(1024, 1024);
	...
	void CloudsModel::generateWeatherMap() {
		chif::utils::bindTexture2D(weatherTex, 0);
		weatherShader->use();
