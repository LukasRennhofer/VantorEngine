...
namespace Vantor::Platform::Input
{
    ...
    {
            bool buttons[BUTTON_ENUM_SIZE] = {}; // it contains pressed buttons as "keyboard/typewriter"
                                                 // like, so no continuous presses
    };

    struct MouseState
    {
            Vantor::Helpers::Math::Vec2 position            = Vantor::Helpers::Math::Vec2(0, 0);
            Vantor::Helpers::Math::Vec2 delta_position      = Vantor::Helpers::Math::Vec2(0, 0);
            float                       delta_wheel         = 0;
            float                       pressure            = 1.0f;
    ...
    {
            uint32_t                    buttons      = 0;
            Vantor::Helpers::Math::Vec2 thumbstick_L = Vantor::Helpers::Math::Vec2(0, 0);
            Vantor::Helpers::Math::Vec2 thumbstick_R = Vantor::Helpers::Math::Vec2(0, 0);
            float                       trigger_L    = 0;
            float                       trigger_R    = 0;
    };
    ...
    {
            float vibration_left = 0;         // left vibration motor (0: no vibration,
                                              // 1: maximum vibration)
            float vibration_right = 0;        // right vibration motor (0: no
                                              // vibration, 1: maximum vibration)
            Vantor::Helpers::Color led_color; // led color
    };
