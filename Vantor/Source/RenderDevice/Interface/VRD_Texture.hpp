 ...
 *  Last Change: Implemented VTexture interface and related types
 */
...
namespace Vantor::RenderDevice
{
    ...
    class VTexture
    {
        public:
            ...
            virtual bool SetData(const void* data, size_t dataSize, uint32_t level = 0, 
                               VTextureDataType dataType = VTextureDataType::UnsignedByte) = 0;
            // Upload pixel data to a region of the texture
            virtual bool SetSubData(const void* data, size_t dataSize, 
                                  uint32_t x, uint32_t y, uint32_t width, uint32_t height,
                                  uint32_t level = 0, VTextureDataType dataType = VTextureDataType::UnsignedByte) = 0;
    ...
    class VTexture2D : public VTexture
    {
        public:
            ...
            static std::shared_ptr<VTexture2D> CreateFromFile(const std::string& filePath, 
                                                            const VTextureSampler& sampler = {},
                                                            bool generateMipmaps = true);
            // Create with pixel data
            static std::shared_ptr<VTexture2D> CreateFromMemory(const void* data, uint32_t width, uint32_t height,
                                                              VTextureFormat format = VTextureFormat::RGBA8,
                                                              const VTextureSampler& sampler = {});

            // Create empty texture (for RenderTargets)
            static std::shared_ptr<VTexture2D> CreateEmpty(uint32_t width, uint32_t height,
                                                         VTextureFormat format = VTextureFormat::RGBA8,
                                                         const VTextureSampler& sampler = {});
    };
    ...
    class VTextureCube : public VTexture
    {
        public:
            ...
            virtual bool SetFaceData(uint32_t face, const void* data, size_t dataSize,
                                   uint32_t level = 0, VTextureDataType dataType = VTextureDataType::UnsignedByte) = 0;

            // Create from file
            static std::shared_ptr<VTextureCube> CreateFromFiles(const std::string faceFiles[6],
                                                               const VTextureSampler& sampler = {});
    };
