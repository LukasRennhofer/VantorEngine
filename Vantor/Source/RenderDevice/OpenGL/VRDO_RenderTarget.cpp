...
namespace Vantor::RenderDevice {
    ...
    VOpenGLRenderTarget::VOpenGLRenderTarget()
        : m_FramebufferID(0), m_PreviousFramebufferID(0)
    {
    }
    ...
    VOpenGLRenderTarget::VOpenGLRenderTarget(VOpenGLRenderTarget&& other) noexcept
    {
    ...
    bool VOpenGLRenderTarget::Create(const VRenderTargetDescriptor& descriptor)
    {
        ...
        Vantor::Backlog::Log("VOpenGLRenderTarget", 
                           "Created render target: " + m_DebugName + " (" + 
    ...
    bool VOpenGLRenderTarget::IsBound() const
    {
        return m_IsBound;
    }
    ...
    void VOpenGLRenderTarget::Clear(const VClearValues& clearValues)
    {
        ...
        if (!m_ColorTextures.empty()) {
            glClearColor(clearValues.ColorValue.x, clearValues.ColorValue.y, 
                        clearValues.ColorValue.z, clearValues.ColorValue.w);
        ...
        switch (attachment) {
            ...
            case VEAttachmentType::Color3: {
                ...
                if (colorIndex < m_ColorTextures.size()) {
                    float color[4] = {clearValues.ColorValue.x, clearValues.ColorValue.y, 
                                    clearValues.ColorValue.z, clearValues.ColorValue.w};
    ...
    bool VOpenGLRenderTarget::IsResizable() const
    {
        return m_IsResizable;
    }

    uint32_t VOpenGLRenderTarget::GetWidth() const
    {
        return m_Width;
    }

    uint32_t VOpenGLRenderTarget::GetHeight() const
    {
        return m_Height;
    }

    float VOpenGLRenderTarget::GetAspectRatio() const
    {
        return m_Height > 0 ? static_cast<float>(m_Width) / static_cast<float>(m_Height) : 1.0f;
    }

    const std::string& VOpenGLRenderTarget::GetDebugName() const
    {
        return m_DebugName;
    }
    ...
    bool VOpenGLRenderTarget::IsValid() const
    {
        return m_FramebufferID != 0 && glIsFramebuffer(m_FramebufferID) == GL_TRUE;
    }
    ...
    std::shared_ptr<VTexture> VOpenGLRenderTarget::GetDepthTexture() const
    {
        return m_DepthTexture;
    }

    std::shared_ptr<VTexture> VOpenGLRenderTarget::GetStencilTexture() const
    {
    ...
    bool VOpenGLRenderTarget::IsMultisampled() const
    {
    ...
    {
        switch (type) {
            case VEAttachmentType::Color0: return GL_COLOR_ATTACHMENT0;
            case VEAttachmentType::Color1: return GL_COLOR_ATTACHMENT1;
            case VEAttachmentType::Color2: return GL_COLOR_ATTACHMENT2;
            case VEAttachmentType::Color3: return GL_COLOR_ATTACHMENT3;
            case VEAttachmentType::Depth: return GL_DEPTH_ATTACHMENT;
            case VEAttachmentType::Stencil: return GL_STENCIL_ATTACHMENT;
            case VEAttachmentType::DepthStencil: return GL_DEPTH_STENCIL_ATTACHMENT;
    ...
    {
        VRenderTargetDescriptor CreateColorTarget(uint32_t width, uint32_t height, 
                                                 VTextureFormat format, const std::string& debugName)
        {
        ...
        VRenderTargetDescriptor CreateColorDepthTarget(uint32_t width, uint32_t height,
                                                      VTextureFormat colorFormat, VTextureFormat depthFormat,
        ...
        VRenderTargetDescriptor CreateHDRTarget(uint32_t width, uint32_t height,
                                               VTextureFormat hdrFormat, const std::string& debugName)
        {
            return CreateColorTarget(width, height, hdrFormat, debugName);
        }

        VRenderTargetDescriptor CreateMRTTarget(uint32_t width, uint32_t height,
                                               const std::vector<VTextureFormat>& colorFormats,
                                               VTextureFormat depthFormat, const std::string& debugName)
        {
            VRenderTargetDescriptor desc;
            desc.Width = width;
            desc.Height = height;
    ...
    {
        std::unique_ptr<VOpenGLRenderTarget> CreateColorTarget(uint32_t width, uint32_t height, 
                                                              VTextureFormat format, const std::string& debugName)
        {
        ...
        std::unique_ptr<VOpenGLRenderTarget> CreateColorDepthTarget(uint32_t width, uint32_t height,
                                                                   VTextureFormat colorFormat, VTextureFormat depthFormat,
                                                                   const std::string& debugName)
        {
            auto renderTarget = std::make_unique<VOpenGLRenderTarget>();
        ...
        std::unique_ptr<VOpenGLRenderTarget> CreateHDRTarget(uint32_t width, uint32_t height,
                                                            VTextureFormat hdrFormat, const std::string& debugName)
        {
            return CreateColorTarget(width, height, hdrFormat, debugName);
        }

        std::unique_ptr<VOpenGLRenderTarget> CreateShadowMapTarget(uint32_t size, VTextureFormat depthFormat,
                                                                  const std::string& debugName)
        {
            auto renderTarget = std::make_unique<VOpenGLRenderTarget>();
