...
namespace Vantor::Core::Memory
{
    ...
    template <typename T, size_t block_size = 256> struct BlockAllocator
    {
            ...
            inline bool is_empty() const { return (blocks.size() * block_size) == free_list.size(); }
    };
    ...
    struct PageAllocator
    {
            ...
            //	deferred_release	:	if false, allocations are freed immediately (suitable for CPU only allocations), otherwise they are freed after a number
            //of frames passed (which should be used for GPU allocations)
