...
namespace Vantor::Core
{
    ...
    template <typename T, typename A = std::allocator<T>> class Vector
    {
        public:
            ...
            inline Vector(size_t initial_size = 0) { resize(initial_size); }

            // Copy constructor - creates deep copy of another Vector
            inline Vector(const Vector<T> &source_Vector) { copy_from(source_Vector); }

            // Move constructor - transfers ownership from another Vector
            inline Vector(Vector<T> &&source_Vector) noexcept { move_from(std::move(source_Vector)); }
