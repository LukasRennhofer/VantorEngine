...
namespace vantor::Graphics
{
    unsigned int TextureFromFile(const char *path, const string &directory, bool gamma)
    {
        ...
        else
        {
            ...
            vantor::Backlog::Log("Texture", errorMessage, vantor::Backlog::LogLevel::ERR);
            stbi_image_free(data);
        }
    ...
    unsigned int loadCubemap(vector<std::string> faces)
    {
        ...
        for (unsigned int i = 0; i < faces.size(); i++)
        {
            ...
            {
                vantor::Backlog::Log("Texture", "Cubemap texture failed to load", vantor::Backlog::LogLevel::ERR);
                stbi_image_free(data);
            }
        }
    ...
    void bindTexture2D(unsigned int tex, int unit) { glBindImageTexture(unit, tex, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA32F); }
} // NAMESPACE vantor::Graphics