...
namespace vantor::GUI
{
    ...
    FontAtlas::FontAtlas(FT_Face face, int pixelSize) : _face(face), _width(0), _height(0)
    {
        _slot = _face->glyph;
        ...
        for (int i = 32; i < 128; ++i)
        {
            ...
            {
                vantor::Backlog::Log("Font", "Loading character %c failed!", vantor::Backlog::LogLevel::ERR);
                continue; // try next character
            }

            _width += _slot->bitmap.width + 2; // add the width of this glyph to our texture width
            // Note: We add 2 pixels of blank space between glyphs for padding -
            // this helps reduce texture bleeding
        ...
        for (int i = 32; i < 128; ++i)
        {
            if (FT_Load_Char(_face, i, FT_LOAD_RENDER)) continue;
