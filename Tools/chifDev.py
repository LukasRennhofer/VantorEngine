...
"""
============================================================================ INFO ===============================================================================================
This module is used to build a CHIFEngine project and its baselib. It uses the cmake build system and the make compile manager to compile the source code and create the final libaries and executables.
It is designed to be run from the command line (TODO: Can support GUI with CHIFDevGUI) and takes care of setting up the environment, compiling the source code, and linking the final 
executables.
"""
    ...
    def show_loading_indicator(process, task_name):
        ...
        while True:
            ...
            if retcode is not None:
                if retcode == 0:
                    sys.stdout.write(f"\r{CHIFDevConsoleUtils.colorGreen}✅ Complete!{" " * (len(task_name) + 5)}{CHIFDevConsoleUtils.coloReset}\n")
                else:
                    sys.stdout.write(f"\r{CHIFDevConsoleUtils.colorGreen}❌ Failed!{" " * (len(task_name) + 8)}{CHIFDevConsoleUtils.coloReset}\n")
                break

            dot_count = int(time.time() * 10) % dots
            dots_str = " ".join([CHIFDevConsoleUtils.colorPurple + "•" + CHIFDevConsoleUtils.coloReset if j <= dot_count else "•" for j in range(dots)])
            sys.stdout.write(f"\r{CHIFDevConsoleUtils.colorGreen}{task_name}...{CHIFDevConsoleUtils.coloReset} [{dots_str}]")
            sys.stdout.flush()
            time.sleep(0.1)

# ======================= Internal Build System (For the Libaries and Includes, neeeds to be updated @LRDev - 4.3) =================
class CHIFEngineInternalBuildSystem:
    """
    CHIFEngineInternalBuildSystem handles the build process for CHIFEngine, including library compilation,
    ...
    def buildLib(self, debugging=False, target=None):
        ...
        CHIFDevConsoleUtils.show_loading_indicator(result, "Building Cmake Configs")
        ...
        CHIFDevConsoleUtils.show_loading_indicator(result, "Building the library")
    ...
    def buildExample(self, target, exampleName, debugging=False):
        ...
        CHIFDevConsoleUtils.show_loading_indicator(result, f"Building {exampleName}")
...
class CHIFDevConsole:
    def __init__(self):
        self.build_system = CHIFEngineInternalBuildSystem()
    
    def parseArgs(self):
        parser = argparse.ArgumentParser(description="CHIFEngine Dev Environment")
        ...
        parser.add_argument(
            "--build-lib", 
            action="store_true", 
            help="Build the CHIFEngine libaries"
        )
